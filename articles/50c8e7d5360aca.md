---
title: "gitbucket-commitgraphs-plugin を修正しようとしたらハマった話"
emoji: "🦁"
...
[GitBucket](https://gitbucket.github.io/) のプラグイン [gitbucket-commitgraphs-plugin](https://github.com/yoshiyoshifujii/gitbucket-commitgraphs-plugin) は、開発者のコミット状況をグラフで表示するプラグインです。

グラフの描画には、[Google Charts](https://developers.google.com/chart) を利用しているため、インターネットに接続できないクローズド環境では、使用できません。

あまり需要はないかもしれませんが、個人的にクローズド環境に対応させたく、修正を試みたのですが、謎の依存関係にハマってしまったというお話です。
...
本当は、本家にコントリビュートできるようにしたかったのですが、後述の事情により、[GitBucket Commit Graphs Chart.js Plugin](https://github.com/yasumichi/gitbucket-commitgraphs-chartjs-plugin) という別のリポジトリを作成しました。

バージョン [v0.1.0](https://github.com/yasumichi/gitbucket-commitgraphs-chartjs-plugin/releases/tag/v0.1.0) としてリリースしましたのでお試しいただけますと幸いです。

[Google Charts](https://developers.google.com/chart) の代わりに [chart.js](https://www.chartjs.org/) を同梱し、利用するようにしました。

オリジナル版では、以下のように表示されていました。
...
なお、日付の表示は、ブラウザの言語設定により変わります。

## 謎の依存関係

[gitbucket-commitgraphs-plugin](https://github.com/yoshiyoshifujii/gitbucket-commitgraphs-plugin) を [Fork](https://github.com/yasumichi/gitbucket-commitgraphs-plugin) し、clone してビルドしようとしたら、以下のエラーに遭遇しました。
...
[java-diff-utils](https://java-diff-utils.github.io/java-diff-utils/) の `groupId` が変わってしまったのでダウンロードできないのだと思いますが、そもそもなぜ java-diff-utils への依存を拾ってくるのから、見当がつきません。

java-diff-utils という文字列でプロジェクトないを検索してみましたが、当該ログが引っかかるのみです。

## 依存関係ツリーを表示してみる

`sbt dependencyTree` で依存関係を調べてみました。
...
やはり、java-diff-utils は見つかりません。

スコープを変えて `sbt runtime:dependencyTree` を実行してみます。
...
さらに `sbt test:dependencyTree` を実行してみます。
...
`sbt compile:dependencyTree` でも状況は変わりません。
...
以下のバージョンを変えてみたり、キャッシュを削除したりしてみましたが、現象は変わりませんでした。
...
## 解決できないので新規 sbt プロジェクトを作成し src 以下を移植してみる

gitbucket-commitgraphs-chartjs-plugin というディレクトリを作成し、以下のファイルを作成しました。
...
gitbucket-commitgraphs-plugin から src をそくっりもらってきます。

それから、`sbt compile` を実行しました。
...
deprecations の警告は出るもののすんなりコンパイルが通りました。なんでやねん！

## Google Charts の処理部分を調査する

手始めに [src/main/twirl/me/huzi/gitbucket/commitgraphs/list.scala.html の28行目](https://github.com/yoshiyoshifujii/gitbucket-commitgraphs-plugin/blob/7427e3ffb2debf30f49eb72c79547e93e6dc7887/src/main/twirl/me/huzi/gitbucket/commitgraphs/list.scala.html#L28) から始まる以下の処理が、グラフの描画を担っています。
...
## chart.js など必要なファイルを同梱するディレクトリを作成

ディレクトリ [src/main/resources/commitgraphs-chartjs/assets](https://github.com/yasumichi/gitbucket-commitgraphs-chartjs-plugin/tree/main/src/main/resources/commitgraphs-chartjs/assets) を掘り、以下のファイルを配置しました。
...
[chartjs-adapter-luxon](https://github.com/chartjs/chartjs-adapter-luxon) は、[luxon](https://moment.github.io/luxon/#/)により、X軸に表示する日付をブラウザのローケルに合わせて表示するための [Date Adapters](https://www.chartjs.org/docs/latest/axes/cartesian/time.html#date-adapters) です。

## assetsMappings を設定

[src/main/scala/Plugin.scala](https://github.com/yasumichi/gitbucket-commitgraphs-chartjs-plugin/blob/f131c6cfcee5203d5f379c23c49d6df1b1d24ea0/src/main/scala/Plugin.scala#L22) で `assetsMappings` を override し、同梱したファイルを使えるようにします。

```scala:src/main/scala/Plugin.scala
override val assetsMappings = Seq("/commitgraphs-chartjs" -> "/commitgraphs-chartjs/assets")
```

## chart.js を使用してグラフを描画するように修正

[src/main/twirl/me/huzi/gitbucket/commitgraphs/list.scala.html](https://github.com/yasumichi/gitbucket-commitgraphs-chartjs-plugin/blob/f131c6cfcee5203d5f379c23c49d6df1b1d24ea0/src/main/twirl/me/huzi/gitbucket/commitgraphs/list.scala.html) を修正しました。

まず、同梱した JavaScript を読み込むようにします。
...
[Google Charts](https://developers.google.com/chart) では X軸の値とY軸の値が対になっていましたが、 [chart.js](https://www.chartjs.org/) では、X軸とY軸を別の配列で定義する必要があります。
...
実はこの部分、Scala というか Java 的には、deprecations なコードが入っています。(java.util.Date の getYearなど)

次にY軸の値を定義するとともに先ほど定義したX軸を関連付けます。オリジナルに合わせて、グラフを塗りつぶすように `fill` を `true` にしています。
...
- `locale` を `navigator.language` から取得
- X軸の `type` を `time` にすることで日付間隔を調整

Copilot からサジェストされた plugins はなくても良かったかな…
...
オリジナルのまま、タグの id をそのまま使ってますが、div から canvas に変えたので合わせた方が良かったかな…

## CommitGraphsSettingsService の deprecations な記述を修正

[src\main\scala\me\huzi\gitbucket\commitgraphs\service\CommitGraphsSettingsService.scala](https://github.com/yoshiyoshifujii/gitbucket-commitgraphs-plugin/blob/7427e3ffb2debf30f49eb72c79547e93e6dc7887/src/main/scala/me/huzi/gitbucket/commitgraphs/service/CommitGraphsSettingsService.scala#L13) に deprecations な記述があると警告が出ました。

例えば、以下の部分の２行目にある `defining` という記述です。元々、GitBucket に用意されていたシンタックスシュガーのようです。

```scala:src\main\scala\me\huzi\gitbucket\commitgraphs\service\CommitGraphsSettingsService.scala
...
[改造版](https://github.com/yasumichi/gitbucket-commitgraphs-chartjs-plugin/blob/main/src/main/scala/me/huzi/gitbucket/commitgraphs/service/CommitGraphsSettingsService.scala#L13) では、以下のように修正しています。
